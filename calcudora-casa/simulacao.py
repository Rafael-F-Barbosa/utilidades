...
from matematica import mensal_rate_from_annual_effective, gerar_tabela_sac as _gerar_tabela_sac


@st.cache_data
def gerar_tabela_sac(financiado: float, n_periodos: int, i_mensal: float,):
    return _gerar_tabela_sac(financiado, n_periodos, i_mensal)

def main():
    st.title("Simulação de Financiamento Imobiliário — SAC")

    st.sidebar.header("Parâmetros do financiamento")
    valor_imovel = st.sidebar.number_input("Valor do imóvel (R$)", value=282000.0, step=1000.0, format="%.2f")
    ...
    i_mensal = mensal_rate_from_annual_effective(cet)
    st.write(f"Taxa efetiva mensal aproximada: {i_mensal * 100:.4f}%")

    df = gerar_tabela_sac(financiado, n_periodos, i_mensal)
    ...
    st.markdown(
        ...
        - $J_t$: juros do período $t$ (R$) — $J_t = S_{t-1} . i_m$
        - $CESH$: Custo Efetivo Seguro Habitacional (percentual a.a.)
        - $i_{seg,m}$: taxa mensal do seguro — $i_{seg,m} = CESH / 12$
        - $Seguro_t$: valor do seguro no período $t$ (R$) — $Seguro_t = S_{t-1} . i_{seg,m}$
